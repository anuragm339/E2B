...
class DataRefreshContextSpec extends Specification {

    def "recordResetAck marks consumer replaying and offset 0"() {
        given:
        def context = new DataRefreshContext("topic", ["groupA:topic"] as Set)
    ...
    def "all reset and ready acks checks reflect expected consumers"() {
        given:
        def context = new DataRefreshContext("topic", ["groupA:topic", "groupB:topic"] as Set)
