...
@Singleton
public class RefreshStateStore {
    private static final Logger log = LoggerFactory.getLogger(RefreshStateStore.class);
    private static final String STATE_FILE_NAME = "data-refresh-state.properties";

    private final Path stateFilePath;

    public RefreshStateStore(@Value("${broker.storage.data-dir:./data}") String dataDir) {
     ...
     */
    public synchronized void saveState(RefreshContext context) {
        ...
        for (int i = 0; i < downtimePeriods.size(); i++) {
            RefreshContext.DowntimePeriod period = downtimePeriods.get(i);
     ...
     */
    public synchronized Map<String, RefreshContext> loadAllRefreshes() {
        Map<String, RefreshContext> contexts = new HashMap<>();
        ...
        try (InputStream in = Files.newInputStream(stateFilePath)) {
            ...
            for (String topic : activeTopics) {
                try {
                    RefreshContext context = loadTopicState(props, topic);
                    if (context != null) {
     ...
     */
    private RefreshContext loadTopicState(Properties props, String topic) {
        ...
        RefreshState state = RefreshState.valueOf(stateStr);
     ...
     */
    @Deprecated
    public synchronized RefreshContext loadState() {
        if (!Files.exists(stateFilePath)) {
            return null;
        }
