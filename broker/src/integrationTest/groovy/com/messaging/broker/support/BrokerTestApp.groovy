...
class BrokerTestApp {
    ...
    static BrokerTestApp start(Path dataDir, Map<String, ?> extraProps = [:]) {
    ...
    ModernConsumerClient connectModernConsumer() {
        def consumer = ModernConsumerClient.connect('127.0.0.1', port)
        managedClients.add(consumer)
        consumer
    }

    LegacyConsumerClient connectLegacyConsumer(String serviceName) {
        def consumer = LegacyConsumerClient.connect('127.0.0.1', port, serviceName)
        managedClients.add(consumer)
        consumer
    }
