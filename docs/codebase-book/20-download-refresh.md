...
- **Fresh install / empty boot (P3, implemented):** on startup `BrokerService` detects a fresh node — `storage.getTopicNames()` empty **and** no `data-refresh-state.properties` (refresh-recovery owns that case) **and** no `pipe-offset.properties` — and, when `broker.bootstrap.fresh-install.enabled` (default true; **false in tests**), runs the **same** bootstrap a bare-metal converges to instead of silently streaming in while `/health` reads green:
  ...
  **Metrics across a broker restart.** `DataRefreshMetrics` is in-memory, so a restart wipes every refresh gauge; the dashboard panels recover *historical* values from Prometheus via `max_over_time(metric[$__range:])`, but the broker's *live* exposition resets. Recovery (`RefreshRecoveryService`) now restores continuity: (1) `refresh.type` is **persisted** in `data-refresh-state.properties` and restored into the rebuilt `RefreshContext` — previously a recovered refresh defaulted to `LOCAL`, mislabeling its `refresh_type`-tagged metrics into a separate series **and** dropping the settled/drained gate (which keys off the type); (2) `repopulateMetricTimings` re-emits `data_refresh_start_time_seconds` (from the persisted reset-sent time as the start proxy — `start.time` is final on the context) and re-seeds `data_refresh_messages_transferred_total` from each consumer's `current.offset − replay.start.offset`, via `DataRefreshMetrics.restoreRefreshStartTime` / `restoreMessagesTransferred` (set, not increment; no `refreshStartedTotal` bump). Whether the recovered refresh ends as a *completed* row still depends on consumers reconnecting after the restart — see the consumer-no-reconnect-after-EOF limitation above.
